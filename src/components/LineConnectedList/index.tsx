    ...
    {items.map((item, index) => {
      return (
        <div key={index}>
          ...
          {index <= (items.length-2) && <div style={{margin: '-2px 0 -2px 10px'}}>
            <span style={{minHeight: 28, borderLeftStyle: 'solid', borderLeftWidth: 1, display: 'block', borderColor: '#bdbdbd'}}></span>
          </div>}
        </div>
      )
    })}
